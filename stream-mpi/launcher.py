...
def start_mpi(extra_args=None):
    ...
    if extra_args is None: # important for gmx
        MASTER_CMD = "mpiexec --mca ess_base_forward_signals SIGTERM --mca odls_base_sigkill_timeout 20 --allow-run-as-root -wdir /home/hpc-tests/stream-mpi/ --host " +  str(MPI_HOST) + " -np " + str(getNumberOfRanks()) + " stream_mpi 100000000 100"
    else:
        MASTER_CMD = "mpiexec --mca ess_base_forward_signals SIGTERM --mca odls_base_sigkill_timeout 20 --allow-run-as-root -wdir /home/hpc-tests/stream-mpi/ --host " +  str(MPI_HOST) + " -np " + str(getNumberOfRanks()) + " stream_mpi 100000000 100 " + str(extra_args)
